...
CustomerContactForm = get_class('customer.forms', 'CustomerContactForm')
...
# Contact view
# ------------
class CustomerContactView(generic.FormView):
    template_name = 'customer/contact/customer_contact.html'
    form_class = CustomerContactForm
    success_url = reverse_lazy('customer:contact')
    communication_type_code = 'CONTACT_EMAIL'
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            engine_info = form.cleaned_data.get('engine_info')
            ...
            msgs = CommunicationEventType.objects.get_and_render(
                code=self.communication_type_code, context=ctx)
            msgs['subject'] = 'Contact Email'

            # Todo: config email
            Dispatcher().dispatch_direct_messages(email, msgs)
            messages.success(self.request,
                             _("Your message was sent successfuly!"))
